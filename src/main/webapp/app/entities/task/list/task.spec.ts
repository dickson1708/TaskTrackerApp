...
describe('Task Management Component', () => {
  ...
  it('should infinite scroll', async () => {
    ...
    req.flush([{ id: 25192 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
    expect(comp.tasks().length).toEqual(1);
    expect(comp.tasks()[0]).toEqual(expect.objectContaining({ id: 25192 }));

    // WHEN
    comp.loadNextPage();
    TestBed.tick();
    expect(service.tasksParams()).toMatchObject(expect.objectContaining({ page: '1' }));
    ...
    await vitest.runAllTimersAsync();
    expect(comp.tasks().length).toEqual(2);
    expect(comp.tasks()[1]).toEqual(expect.objectContaining({ id: 22244 }));

    comp.loadNextPage();
    TestBed.tick();
    expect(service.tasksParams()).toMatchObject(expect.objectContaining({ page: '2' }));
    req = httpMock.expectOne({ method: 'GET' });
    req.flush([{ id: 22244 }], {
      headers: { link: '<http://localhost/api/foo?page=0&size=20>; rel="prev",<http://localhost/api/foo?page=2&size=20>; rel="next"' },
    });
